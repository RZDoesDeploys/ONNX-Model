...
async function preprocessImage(imagePath) {
  ...
  for (let i = 0; i < 224 * 224; i++) {
    chwData[i] = normalizedData[i * 3];
    chwData[i + 224 * 224] = normalizedData[i * 3 + 1];
    chwData[i + 2 * 224 * 224] = normalizedData[i * 3 + 2];
  }
...
// Predict
async function predict(imagePath, modelPath = './model.onnx') {
...
if (!args.length) {
  console.error('⚠️  Please provide an image path.\nUsage: node index.js <image-path>');
  process.exit(1);
}
predict(args[0]);
